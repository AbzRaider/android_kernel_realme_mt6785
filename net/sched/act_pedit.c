		     ...
		     struct tcf_result *res)
{
	...
	if (p->tcfp_nkeys > 0) {
		...
		for (i = p->tcfp_nkeys; i > 0; i--, tkey++) {
			...
			if (rc) {
				pr_debug("tc filter pedit bad header type specified (0x%x)\n",
					htype);
				goto bad;
			}
			...
			if (tkey->offmask) {
				...
				if (!offset_valid(skb, hoffset + tkey->at)) {
					pr_debug("tc filter pedit 'at' offset %d out of bounds\n",
						hoffset + tkey->at);
					goto bad;
				}
			...
			if (offset % 4) {
				pr_debug("tc filter pedit"
					" offset must be on 32 bit boundaries\n");
				goto bad;
			}

			if (!offset_valid(skb, hoffset + offset)) {
				pr_debug("tc filter pedit offset %d out of bounds\n",
					hoffset + offset);
				goto bad;
			}
			...
			case TCA_PEDIT_KEY_EX_CMD_ADD:
				...
				break;
			default:
				pr_debug("tc filter pedit bad command (%d)\n",
					cmd);
				goto bad;
			}
