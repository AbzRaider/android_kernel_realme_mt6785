...
retry:
	...
	if ((get_project() == 17197) || (get_project() == 18311) || (get_project() == 18011) || (get_project() == 18531) ||
		...
		if(reg == 0x94) {/* tfa9894 */
			...
			sprintf(fw_name,"tfa/tfa/tfa98xx.cnt");
			sprintf(fw_comm_name,"tfa/tfa/common/tfa98xx.cnt");
		}
	} else {/* tfa9890 */
			sprintf(fw_name,"tfa/tfa/tfa98xx.cnt");
			sprintf(fw_comm_name,"tfa/tfa/common/tfa98xx.cnt");
	...
	pr_debug("%s: ID revision 0x%04x, fw_name is %s, fw_comm_name is %s\n", __func__, reg, fw_name, fw_comm_name);
#endif /* VENDOR_EDIT */
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (!value) {
		...
		pr_debug("%s force the change to calibration profile for calibrate\n", __func__);
	}
	#endif /* VENDOR_EDIT */
