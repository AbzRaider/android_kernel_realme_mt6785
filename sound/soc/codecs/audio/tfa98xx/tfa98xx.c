...
struct tfa98xx *tfa98xx_whole_v6;
 ...
 #ifdef OPLUS_ARCH_EXTENDS
// Yongpei.Yao@PSW.MM.AUDIODRIVER.PA, 2020/10/22, Add for spk force mute
...
#ifdef VENDOR_EDIT
//xiang.fei@PSW.MM.AudioDriver.HAL, 2019/03/26,
...
#ifdef VENDOR_EDIT
/* Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.954616,
...
#ifdef OPLUS_ARCH_EXTENDS
// Yongpei.Yao@PSW.MM.AUDIODRIVER.PA, 2020/10/20, Add for spk force mute
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("Speaker reboot set to %s\n", val == 1 ? "on" : "off");

	if (TFA_STATE_OPERATING == tfa_dev_get_state(tfa98xx->tfa)) {
		pr_debug("TFA operating, setting state\n");
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (val == s_speaker_force_mute) {
		pr_debug("Speaker mute is already %s\n", val == 1 ? "on" : "off");
		return 1;
	}

	pr_debug("Speaker mute set to %s\n", val == 1 ? "on" : "off");
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (err)
		pr_debug("%s: set volume error, code:%d\n", __func__, err);
	else
		pr_debug("%s: set volume ok\n", __func__);
	mutex_unlock(&tfa98xx->dsp_lock);
...
static int tfa98xx_ana_volume_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: get ana volume ok, ana volume: %d\n", __func__, tfa98xx_ana_vol);
	mutex_unlock(&tfa98xx->dsp_lock);
...
/*Zhao.Pan@PSW.MM.AudioDriver.SmartPA, 2020/01/08, Add for stereo spk*/
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx *tfa98xx_cal, int *speakerImpedance, bool b_aging)
{
    ...
    pr_debug("check_hw_power success\n");
	...
	if (tfa98xx->tfa->dev_idx == 0) {
		...
		pr_debug("Calibration started profile %d\n", profile);
		...
		for (i=0; i < 50; i++ ) {
			...
			*/
			if(buffer[2] & 0x6) {
				if (buffer[2] & 0x2)
					pr_debug("%s: ##ERROR## Primary SPK damaged event detected 0x%x\n", __func__, buffer[2]);
				if (buffer[2] & 0x4)
					pr_debug("%s: ##ERROR## Second SPK damaged event detected 0x%x\n", __func__, buffer[2]);
				break;
			}
		}
		...
		if (tfa98xx_device_count == 2) {
			...
			imp = (tfa_get_calibration_info(tfa98xx->tfa, 1));
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				if (tfa98xx->tfa->dev_idx==1)
					tfa98xx->tfa->mohm[0] = imp;
			}
			/* restore the org point*/
			tfa98xx = tfa98xx_cal;
		}
		...
		*speakerImpedance = tfa_get_calibration_info(tfa98xx->tfa, 0);
		...
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			...
			if(imp == 0) {
				...
				imp = 6100;
				pr_debug("%s: set the default %d/1000 ohm to MTP, due to SPK was damage\n", __func__, imp);
			}
		...
		*speakerImpedance = tfa_get_calibration_info(tfa98xx->tfa, 0);
		pr_debug("%s:  devid 1 Calibration value: %d.%3d ohm.\n", __func__, *speakerImpedance/1000, *speakerImpedance%1000);
	}

	return Tfa98xx_Error_Ok;
}
#else //VENDOR_EDIT && CONFIG_TFA9874_NONDSP_STEREO
	...
	struct tfa98xx *tfa98xx, int *speakerImpedance, bool b_aging)
{
	...
	if (tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_EX)) {
		pr_debug("DSP already calibrated, MTPEX==1\n");
		err = tfa_dsp_get_calibration_impedance(tfa98xx->tfa);
		imp = tfa_get_calibration_info(tfa98xx->tfa, 0);
		pr_debug("Calibration value: %d.%3d ohm\n",
	...
	*speakerImpedance = imp;
	pr_debug("%s: Calibration value: %d.%3d ohm\n",
...
#endif //VENDOR_EDIT && CONFIG_TFA9874_NONDSP_STEREO
	...
	if (tfa98xx->tfa->is_probus_device) {
		...
		if (ret) {
			...
			pr_err("[0x%x] calibration failed\n",
				tfa98xx->i2c->addr);
			#ifndef VENDOR_EDIT
			/* Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2018/07/23
				...
				loff_t *ppos)
{
	...
	int ret;
	int speakerImpedance = 0;
	pr_debug("aging calibration start now!\n");
...
/*Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2020/08/29, Add for getting impedance*/
				...
				loff_t *ppos)
{
	...
	pr_debug("tfa_dsp_get_calibration_impedance ret = %d\n", ret);
...
/*Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2020/08/29, Add for checking calibrated or not*/
				...
				loff_t *ppos)
{
	...
	pr_debug("aging check calibrated or not now!\n");
	...
	pr_debug("[0x%x] str: %s", tfa98xx->i2c->addr, str);
...
/*Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/08/31, Add for aging calibration*/
static const struct file_operations tfa98xx_dbgfs_r_aging_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_r_aging_read,
	.llseek = default_llseek,
};
/*Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/08/31, Add for getting impedance*/
static const struct file_operations tfa98xx_dbgfs_r_impedance_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_r_impedance_read,
	.llseek = default_llseek,
};
/*Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/08/31, Add for checking calibrated or not*/
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("g_tfa98xx_firmware_status=%d\n", g_tfa98xx_firmware_status);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (g_tfa98xx_firmware_status > 0) {
		pr_debug("the firmware was loaded, we don't need load it again!!!\n");
		return ret;
	}

	mutex_lock(&tfa98xx_mutex);
	pr_debug("g_tfa98xx_firmware_status=%d    new=%d\n",
		g_tfa98xx_firmware_status,
				...
				int num_dai)
{
	...
	if (dai_drv && num_dai > 0)
		for (i = 0; i < num_dai; i++) {
			...
			pr_debug("dai_drv[%d].playback.stream_name = [%s]\n",
			...
			dai_drv[i].capture.stream_name =
				tfa98xx_devm_kstrdup(dev, buf);
			pr_debug("dai_drv[%d].capture.stream_name = [%s]\n",
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa_is_cold(tfa98xx->tfa) == 0) {
		pr_debug("Warning:device 0x%.2x still warm\n",
			tfa98xx->i2c->addr);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		if ((!value) && (ret == Tfa98xx_Error_Ok)) {
			...
			ret = tfa98xx_tfa_start(tfa98xx,
						tfa98xx->profile,
						tfa98xx->vstep);
			pr_debug("finish calibrate, replay with profile %d, ret %d\n",
				tfa98xx->profile,
				ret);
		}
		#endif /* VENDOR_EDIT */
	...
	if (failed) {
		...
		tfa98xx->init_count = 0;
		#ifdef VENDOR_EDIT
		/* Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.1226731, 2018/05/12
		 * Add for FTM
		 */
		if (ftm_mode == BOOT_MODE_FACTORY)
	...
	if (sync) {
		...
		if (do_sync) {
			...
			list_for_each_entry(tfa98xx,
				...
				tfa_dev_set_state(tfa98xx->tfa,
					TFA_STATE_UNMUTE);
				#ifdef OPLUS_ARCH_EXTENDS
				// Yongpei.Yao@PSW.MM.AUDIODRIVER.PA, 2020/10/20, Add for spk mute ctrl
				if (s_speaker_force_mute == 1) {
					pr_debug("Speaker force mute, muting...\n");
					mdelay(1);
		 ...
		 */
		if (ftm_mode == BOOT_MODE_FACTORY) {
			pr_debug("finish for ftm ringtone\n");
...
//Jianqing.Liao@PSW.MM.AudioDrvier,if impedance < 4ohm, use defalue value 6ohm
			if (value < 4000 && (tfa98xx_factory_flag == 0)) {
				pr_debug("Device 0x%x ,speaker =  %dmohm,use default value.\n",
					tfa98xx->i2c->addr, value);
				value = 6000;
			}
#endif
			dsp_cal_value = (value * 65536) / 1000;
			pr_debug("Device 0x%x cal value is %d\n",
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		/* Start DSP */
		#ifndef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA.1234963,*/
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	if (init_times == 0) {
		...
		pr_debug("We created mixer control in probe  ret=%d\n", ret);
	}
	#else //VENDOR_EDIT
	ret = tfa98xx_create_controls(tfa98xx);
	pr_debug("We created mixer control in probe  ret=%d\n", ret);
	...
	#ifdef VENDOR_EDIT
	/* Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.954616, 2017/01/03
	...
	#ifdef OPLUS_ARCH_EXTENDS
	// Yongpei.Yao@PSW.MM.AUDIODROVER.PA, 2020/10/20, Add for spk force mute
	snd_soc_add_codec_controls(tfa98xx->codec,
...
#ifdef VENDOR_EDIT
#ifdef CONFIG_TFA9874_NONDSP_STEREO
	pr_debug("%s: tfa98xx_device_count = %d\n", __func__, tfa98xx_device_count);
				...
				const struct i2c_device_id *id)
{
	...
	if (!tfa98xx_cache)
	{
		...
		pr_debug("TFA alloc cache success\n");
	}
#endif
