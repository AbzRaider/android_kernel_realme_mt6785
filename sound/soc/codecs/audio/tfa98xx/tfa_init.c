...
void set_ops_defaults(struct tfa_device_ops *ops)
{
	...
	ops->dsp_msg_read = tfa_dsp_msg_v6_read;
...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x1a13) {
		...
		tfa_reg_write(tfa, 0xc6, 0x004e); //POR=0x000e
		/*
		 * PLMA5539: Please make sure bit 6 is always on!
		 */
		/* ----- generated code end   ----- */
			...
			int sample_rate)
{
	...
	/* Write the fractional delay in the */
	/* hardware register 'cs_frac_delay' */
	switch (sample_rate) {
	case 0:	/* 8kHz */
...
{
	/* Set auto_copy_mtp_to_iic (bit 5 of A3) to 1. Workaround for 72, 88 */
	/* and 9912/9892(see PLMA5290) */
...
/* Maksimum value for combination of boost_voltage and vout calibration offset
 */
/* (see PLMA5322, PLMA5528). */
...
static uint16_t tfa9912_vboost_fixup(struct tfa_device *tfa, uint16_t dcdc_cnt6)
{
	...
	/* Get current calibr_vout_offset, this register is not supported by */
	/* bitfields */
	...
	boost_v_1st = tfa_get_bf_v6_value(TFA9912_BF_DCVOF, new_dcdc_cnt6);
	boost_v_2nd = tfa_get_bf_v6_value(TFA9912_BF_DCVOS, new_dcdc_cnt6);
...
/* PLMA5322, PLMA5528 - Limit values of DCVOS and DCVOF to range specified in */
/* datasheet. */
				     ...
				     unsigned short value)
{
	if (subaddress == TFA9912_DCDCCNT6_REG) {
		/* Correct V boost (first and secondary) to ensure 12V is not */
		/* exceeded. */
...
{
	/* Set auto_copy_mtp_to_iic (bit 5 of A3) to 1. Workaround for 72 and 88
	 */
	/* (see PLMA5290) */
	return tfa_reg_write(tfa, 0xA3, 0x20);
}
					       ...
					       int status)
{
	...
	ret = tfa_set_bf_v6_volatile(tfa,
		TFA9874_BF_OPENMTP, (uint16_t)(status));
	return ret;
}
						       ...
						       int sample_rate)
{
	...
	/* Write the fractional delay in the hardware*/
	/* register 'cs_frac_delay' */
	switch (sample_rate) {
	case 0: /* 8kHz */
		TFA_SET_BF(tfa, FRACTDEL, 40);
		break;
...
static enum Tfa98xx_Error tfa9896_specific(struct tfa_device *tfa)
{
	 ...
	 * This will lower the overshoot on IN-B to avoid NMOS damage of
	 * booster.
	 */
	if (tfa->rev == 0x1b96) {
	...
	/* $49:[0] - 1 ==> 0; CLIP - default value changed.*/
	/* 0 means CLIPPER on */
...
static enum Tfa98xx_Error tfa9890_clockgating(struct tfa_device *tfa, int on)
{
	...
	/* for TFA9890 temporarily disable clock*/
	/* gating when dsp reset is used */
						    ...
						    int state)
{
	if (state == 1 || state == 0) {
		if (is_94_N2_device(tfa))
			return -tfa_set_bf(tfa, TFA9894N2_BF_MANAOOSC,
					(uint16_t)state);
		else
			return -tfa_set_bf(tfa, TFA9894_BF_MANAOOSC,
					(uint16_t)state);
	}
