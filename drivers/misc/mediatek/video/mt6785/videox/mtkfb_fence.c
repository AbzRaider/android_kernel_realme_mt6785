				...
				unsigned int idx)
{
	...
	if (!va) {
		/* FIXME: non-ion buffer need cache sync here? */
		pr_err("cannot find this buf, layer=%d, idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
...
static int prepare_ion_buf(struct disp_buffer_info *disp_buf,
 ...
 */
struct mtkfb_fence_buf_info
*disp_sync_prepare_buf(struct disp_buffer_info *disp_buf)
{
	int ret = 0;
	...
	if (disp_buf->ion_fd >= 0)
		prepare_ion_buf(disp_buf, fence_buf);
		...
		list_for_each_entry(buf, &l_info->buf_list, list) {
			...
			if ((buf->mva + buf->mva_offset) == phy_addr) {
				if (phy_addr >= buf->mva &&
					...
					DISP_PR_ERR(
					"wrong addr:pa0x%08lx,mva0x%08lx,sz0x%08x\n",
						    phy_addr, buf->mva,
						    buf->size);
...
#endif
		MTKFB_FENCE_LOG(
		"query buf mva: layer=%d, idx=%d, mva=0x%lx\n",
				timeline_id, fence_idx, buf->mva);
	} else {
		/* FIXME: non-ion buffer need cache sync here? */
		DISP_PR_ERR(
		"cannot find this buf, session:%s%d, layer=%d,\n",
			    disp_session_type_str(session_id),
			    DISP_SESSION_DEV(session_id), timeline_id);
		DISP_PR_ERR(
			"idx=%d, fence_idx=%d, timeline_idx=%d, cur_idx=%d!\n",
