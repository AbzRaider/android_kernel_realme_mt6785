...
struct capi_proxy_param_memref {
	__u64 buffer;
	__u64 size;
	__u64 shm_offset;
	__u64 shm_id;
...
struct capi_proxy_param {
	__u64 attr;
	union {
		struct capi_proxy_param_value	value;
		struct capi_proxy_param_memref	memref;
	};
};

/**
 * struct tee_ioctl_capi_proxy_arg - CAPI proxy driver parameter
 * @uuid:       [in] TA UUID to communication with
 * @capi_op:    [in] CAPI operation: open_session, invoke_command, etc.
 * @session:    [in/out] Session for CAPI operations
 * @func:       [in] command id of invoke_command
 * @ret:        [out] Return values
 * @reg_orig:   [out] Function error origins
 * @params:     [in] TEEC parameters
 */
struct tee_ioctl_capi_proxy_arg {
	...
	__u32 ret;
	__u32 ret_orig;
	struct capi_proxy_param params[4];
} __aligned(8);

/**
 * TEE_IOC_CAPI_PROXY - for communication with CAPI proxy driver
 */
