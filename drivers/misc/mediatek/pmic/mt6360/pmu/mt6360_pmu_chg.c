...
#ifdef VENDOR_EDIT
/* lizhijie@BSP.CHG.Basic, 2020/05/08, lzj add for charger type detect */
#include <linux/delay.h>
#endif

#if defined(CONFIG_OPLUS_HVDCP_SUPPORT) || defined(CONFIG_OPLUS_CHARGER_MTK6785)
/* Kaili.Lu@BSP.CHG.Basic, 2019/09/30,  Add for HVDCP charging */
...
#ifdef VENDOR_EDIT
/* LiYue@BSP.CHG.Basic, 2019/09/11, Add for charging */
	atomic_t suspended;
#endif
#if defined(CONFIG_OPLUS_HVDCP_SUPPORT) || defined(CONFIG_OPLUS_CHARGER_MTK6785)
...
/* Kaili.Lu@BSP.CHG.Basic, 2019/09/30, Add for mate20 hvdcp charging */
...
#endif
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic 2020/04/17 lzj add for support hvdcp*/
	bool support_hvdcp;
#endif
#ifdef VENDOR_EDIT
...
{
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic. 2020/05/19 lzj add for charger*/
	int ret = 0;
#endif
	bool attach = false;
...
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic 2020/05/08 lzj add for charger*/
		if (battery_psy)
...
EXPORT_SYMBOL(is_mtkvooc30_project);
...
/* Kaili.Lu@BSP.CHG.Basic, 2019/09/30,	Add for HVDCP charging */
...
#if defined(CONFIG_OPLUS_HVDCP_SUPPORT) || defined(CONFIG_OPLUS_CHARGER_MTK6785)
	dev_err(mpci->dev, "%s: enable hvdcp detect is_mtksvooc_project = %d\n", __func__, is_mtksvooc_project);
	if (true == is_mtksvooc_project || true == is_mtkvooc30_project) {
		...
		dev_err(mpci->dev,"vooc dpdm drop gnd\n");
	} else {
		ret = mt6360_pmu_reg_write(mpci->mpi, HVDCP_DEVICE_TYPE, 0x0);
		if (ret < 0)
			dev_err(mpci->dev, "%s: fail to write hvdcp_device_type\n", __func__);
...
out:
	...
	/* LiYue@BSP.CHG.Basic, 2019/11/28, add for QC detect */
	if (!attach) {
		cancel_delayed_work_sync(&mpci->hvdcp_work);
		cancel_delayed_work_sync(&mpci->hvdcp_result_check_work);
		mpci->hvdcp_type = POWER_SUPPLY_TYPE_UNKNOWN;
...
static int __mt6360_set_ichg(struct mt6360_pmu_chg_info *mpci, u32 uA)
{
	...
	u32 data = 0;
#ifndef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic. 2020/05/21 lzj add for charger*/
	mt_dbg(mpci->dev, "%s\n", __func__);
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* LuKaili@BSP.CHG.Basic, 2020/08/05, Add for OTG */
	if (true == is_mtksvooc_project) {
...
void oplus_notify_hvdcp_detect_stat(void)
{
	...
	if (mpci && true == is_mtksvooc_project) {
...
static irqreturn_t mt6360_pmu_hvdcp_det_handler(int irq, void *data)
{
	...
	dev_dbg(mpci->dev, "%s\n", __func__);
#if defined(CONFIG_OPLUS_HVDCP_SUPPORT) || defined(CONFIG_OPLUS_CHARGER_MTK6785)
/*lizhijie@BSP.CHG.Basic. 2020/04/17 lzj add for hvdcp support*/
	if (mpci->support_hvdcp == true) {
		cancel_delayed_work_sync(&mpci->hvdcp_result_check_work);
...
#ifdef VENDOR_EDIT
/*Baoquan.Lai@@BSP.CHG.Basic, 2020/07/04, Add for shipmode sequence*/
	else
		ret = 0 ;
#else
	return 0;
...
#ifdef VENDOR_EDIT
/*Baoquan.Lai@@BSP.CHG.Basic, 2020/05/29, Add for MTK ship mode */
...
/*Lukaili@BSP.CHG.Basic, 2020/06/16, Add for qc compatible for vooc project*/
	dev_err(oplusmpci->dev, "%s: enable hvdcp detect is_mtksvooc_project = %d %d\n",
			__func__, is_mtksvooc_project, oplusmpci->hvdcp_exit_stat);
...
#ifdef VENDOR_EDIT
/*lizhijie@BSP.CHG.Basic. 2020/05/07 lzj add for charger*/
...
#ifdef VENDOR_EDIT
//==========Feiping.Li@Camera.Driver, 20200605, add for flashlight ntc temperature=======
...
//==========Gang.Yan@BSP.CHG.BASIC, 20200831, add for chg_mos ntc temperature=======
...
#ifdef VENDOR_EDIT
/* LiYue@BSP.CHG.Basic, 20120/03/03, Add for charging */
	oplusmpci = NULL;
#endif
	return ret;
}
