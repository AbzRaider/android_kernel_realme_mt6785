...
#define LCM_PHYSICAL_WIDTH						(51030)
#define LCM_PHYSICAL_HEIGHT						(90720)
		       ...
		       unsigned int count, unsigned char force_update)
{
	...
	for (i = 0; i < count; i++) {
		...
		default:
			...
			table[i].count, table[i].para_list, force_update);
		}
	}
}
...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.PLL_CLOCK =
		134;	/* this value must be in MTK suggested table */
#else
	params->dsi.PLL_CLOCK =
		127;	/* this value must be in MTK suggested table */
#endif
...
static void lcm_init(void)
{
	...
	if (ret < 0)
		LCM_LOGD("nt35521--tps6132--cmd=%0x--i2c write error--\n",
			cmd);
	else
	...
	if (lcm_dsi_mode == CMD_MODE) {
		LCM_LOGD("nt35521----tps6132 not support cmd mode\n");
	} else {
		push_table(NULL, init_setting,
			sizeof(init_setting) /
			sizeof(struct LCM_setting_table), 1);
		LCM_LOGD("nt35521--tps6132--lcm mode = vdo mode :%d--\n",
			 lcm_dsi_mode);
	}
}
...
static unsigned int lcm_compare_id(void)
{
	...
	LCM_LOGD("%s,nt35521_id=0x%08x,version_id=0x%x\n", __func__, id,
		 version_id);
