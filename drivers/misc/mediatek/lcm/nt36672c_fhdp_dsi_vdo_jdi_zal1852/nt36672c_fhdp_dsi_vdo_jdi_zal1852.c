...
#define LOG_TAG "LCM_NT36672C_JDI_SALA"
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	if (!is_sala_a()) {
		...
		LCM_LOGD("%s,nt sala-A backlight: level = %d\n", __func__, level);
		if (level == 0) {
			bl_level_dimming_exit[1].para_list[0] = (level >> 8) & 0x0F;
			bl_level_dimming_exit[1].para_list[1] = level & 0xFF;
			push_table(handle,
...
LCM_DRIVER nt36672c_fhdp_dsi_vdo_jdi_zal1852_lcm_drv = {
    .name = "nt36672c_jdi_dsjm_sala",
