...
#else
	/* this value must be in MTK suggested table */
	params->dsi.PLL_CLOCK = 566;
#endif
	params->dsi.data_rate = 1131;
...
static unsigned int lcm_esd_check(void)
{
	...
	if (buffer[0] != 0x9C) {
		LCM_LOGD("[LCM ERROR] [0x0A]=0x%02x\n", buffer[0]);
		return TRUE;
	}
	LCM_LOGD("[LCM NORMAL] [0x0A]=0x%02x\n", buffer[0]);
	return FALSE;
#else
	return FALSE;
#endif
...
#if (LCM_DSI_CMD_MODE)
struct LCM_DRIVER nt36672c_fhdp_dsi_cmd_auo_cphy_90hz_lcm_drv = {
	.name = "nt36672c_fhdp_dsi_cmd_auo_cphy_90hz_lcm_drv",
#else
struct LCM_DRIVER nt36672c_fhdp_dsi_vdo_auo_cphy_90hz_lcm_drv = {
	.name = "nt36672c_fhdp_dsi_vdo_auo_cphy_90hz_lcm_drv",
#endif
