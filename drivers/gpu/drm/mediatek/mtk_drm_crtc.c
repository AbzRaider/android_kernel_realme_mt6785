...
/* liwei.a@PSW.MM.DisplayDriver.FP, 2019/11/15, add for power mdoe tag*/
...
//#ifdef VENDOR_EDIT
/*
* LiPing-M@PSW.MM.Display.LCD.Machine, 2019/11/15,
...
//#ifdef OPLUS_BUG_STABILITY
/** LiPing-M@PSW.MM.Display.LCD.Machine, 2019/11/15,add for dc backlight **/
...
static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle, bool en)
{
	...
	//#else
	//use the same cmdq handle with frame flush thread
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	//#ifdef OPLUS_BUG_STABILITY
	/* liwei.a@PSW.MM.Display.LCD.Stability, 2020/10/24, add for stopping mode switch in doze */
...
{
	//#ifdef OPLUS_BUG_STABILITY
	/* liwei.a@PSW.MM.Display.LCD.Stability, 2020/11/21, add for debug*/
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
	...
	unsigned long crtc_id = (unsigned long)drm_crtc_index(crtc);
	//#ifdef OPLUS_FEATURE_ONSCREENFINGERPRINT
	/* LiPing-M@PSW.MM.DisplayDriver.Stability, 2019/12/07, modify for fpd_notify timing */
	//struct cmdq_pkt_buffer *cmdq_buf = &(mtk_crtc->gce_obj.buf);
	//#endif
	struct mtk_drm_private *priv = crtc->dev->dev_private;
#if defined(CONFIG_MACH_MT6873) || defined(CONFIG_MACH_MT6853) || \
...
/* LiPing-M@PSW.MM.DisplayDriver.Stability, 2019/12/07, modify for fpd_notify timing
		cmdq_pkt_mem_move(cmdq_handle, mtk_crtc->gce_obj.base,
				  cmdq_buf->pa_base + DISP_SLOT_FP0_IDX,
				  cmdq_buf->pa_base + DISP_SLOT_FP1_IDX,
				  CMDQ_THR_SPR_IDX3);*/
//#endif
#ifndef CONFIG_FPGA_EARLY_PORTING
    		cmdq_pkt_wait_no_clear(cmdq_handle,
...
/* liwei.a@PSW.MM.DisplayDriver.FP, 2019/11/1, add for fingerprint*/
int hbm_eof_flag = 0;
extern int fingerprint_delay_idx_num;
//#endif
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	//#ifdef OPLUS_FEATURE_ONSCREENFINGERPRINT
	/* liwei.a@PSW.MM.DisplayDriver.FP, 2019/11/10, add for fingerprint*/
	int hbm_en = 0;
	//#endif
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		...
		if (hbm_en == 2) {
			pr_err("now in aod finger unlock\n");
			frame_wait_num = 2;
			fingerprint_delay_idx_num = 2;
		} else if (hbm_en == 1) {
			frame_wait_num = 1;
			fingerprint_delay_idx_num = 1;
		}
		...
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
			...
			if ((hbm_en > 0) && !mtk_crtc_is_frame_trigger_mode(crtc)) {
				...
				if (oppo_fp_notify_down_delay && (hbm_en & 0x2) > 0) {
					/* LiPing-M@PSW.MM.DisplayDriver.Stability, 2019/12/07, modify for fpd_notify timing */
					pf_idx = state->prop_val[CRTC_PROP_PRES_FENCE_IDX];
					addr = cmdq_buf->pa_base + DISP_SLOT_FP0_IDX;
					DDPPR_ERR("%s: write pf_idx to pf0:%u\n",
							__func__, pf_idx);
					cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
								addr, pf_idx, ~0);
					//mtk_drm_crtc_hbm_wait(crtc, 1);
	...
	//#ifdef OPLUS_FEATURE_ONSCREENFINGERPRINT
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
	/* liwei.a@PSW.MM.DisplayDriver.FP, 2019/11/29, modify for hbm set according to dimlayer*/
		...
		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->params
			...
			//#ifdef VENDOR_EDIT
			/* zhangjianbin@PSW.MM.DisplayDriver.FP, 2020/1/10, add a new lcd*/
			if ((hbm_en & 0x2) > 0 && oppo_fp_notify_down_delay && mtk_crtc_is_frame_trigger_mode(crtc)) {
				if (!state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
					oppo_fp_notify_down_delay = false;
				}
