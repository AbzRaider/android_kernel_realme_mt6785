...
static void syna_tcm_dispatch_message(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->status_report_code >= REPORT_IDENTIFY) {
		if ((mod_pool.reconstructing) && (tcm_hcd->status_report_code ==
			REPORT_TOUCH))
			return;
...
static int syna_tcm_enable_irq(struct syna_tcm_hcd *tcm_hcd, bool en, bool ns)
{
	...
	if (en) {
		...
		if (irq_freed) {
			retval = request_threaded_irq(tcm_hcd->irq,
				NULL,
				syna_tcm_isr,
		...
		unsigned long action, void *data)
{
	...
	if (atomic_read(&tcm_hcd->firmware_flashing) && *transition ==
		FB_BLANK_POWERDOWN) {
		...
		if (retval == 0) {
			LOG_ERR(tcm_hcd->pdev->dev.parent,
			"Timed out waiting for completion of flash fw\n");
